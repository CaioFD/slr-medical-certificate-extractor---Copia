...
# Função de processamento simulado (standalone)
def processar_atestado_simulado(arquivo):
    """Simula processamento de atestado para demonstração"""
    try:
        # Simula análise baseada no nome do arquivo
        nome_arquivo = arquivo.name.lower()
        
        # Diferentes resultados baseados no nome do arquivo
        if "atestado" in nome_arquivo or "medical" in nome_arquivo:
...
# Função de registro simplificada (standalone)
def registrar_login_simples(nome_usuario: str):
    """Registro de login simplificado sem banco"""
    try:
...
if not st.session_state["autenticado"]:
    ...
    st.markdown("**Sistema de Validação de Atestados Médicos - Versão Standalone**")
    st.markdown("---")
    ...
    if st.button("🚀 Entrar", type="primary"):
        if email == "admin@teste.com" and senha == "123456" and nome.strip() != "":
            registrar_login_simples(nome.strip())
...
# === INTERFACE PRINCIPAL ===
    ...
    # Status do sistema
    st.info("🤖 Modo Demonstração: Processamento simulado para testes")
    ...
    arquivo = st.file_uploader(
        ...
        help="Formatos aceitos: PDF, JPG, JPEG, PNG - Sistema de demonstração"
    )
    ...
    if arquivo:
        ...
        if st.button("🔍 Validar Atestado", type="primary", use_container_width=True):
            with st.spinner("🔄 Processando documento... Simulando análise..."):
                
                try:
                    # Simula tempo de processamento
                    time.sleep(3)
                    
                    # Processa o arquivo simulado
                    dados_extraidos = processar_atestado_simulado(arquivo)
                    ...
                    if dados_extraidos:
                        ...
                        dados_extraidos.update({
                            ...
                            "observacoes": f"Processamento simulado do arquivo: {arquivo.name}"
                        })
                        ...
                        # Registro de log
                        try:
                            registrar_login_simples(f"Validação: {arquivo.name} - {st.session_state['nome_completo']}")
                        except Exception as e:
        ...
        ⚠️ **Nota:** Esta é uma versão de demonstração com processamento simulado
        """)
    ...
    <div style="text-align: center; padding: 1rem; color: #666;">
        <p>🏥 Sistema de Validação de Atestados Médicos v2.0 (Standalone)</p>
